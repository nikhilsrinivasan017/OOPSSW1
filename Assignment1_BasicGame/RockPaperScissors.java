...
public class RockPaperScissors {
	...
	public boolean isValidTurn(int turn) {
		...
		if ( turn == IROCK || turn == IPAPER || turn == ISCISSORS)
			return true;
		else
			return false;
	}
	...
	public int getPlayerTurn(String input) {
		input = input.toLowerCase(); //in case user enters R/P/S for the game input
		if (input.equals(ROCK_INPUT))
			return 1;
		else if(input.equals(PAPER_INPUT))
			return 2;
		else if(input.equals(SCISSORS_INPUT))
			return 3;
		else
			return -1;
	}
	...
	public String getWinner(int playerChoice, int computerChoice) {
		...
		if(playerChoice == computerChoice)
			...
			if(computerChoice == 2)
				decision = COMPUTER;//rock looses to paper
			else if (computerChoice == 3)
				decision = PLAYER; //rock beats scissors
		}
		else if(playerChoice == 2) { //Player Selects Paper
			if(computerChoice == 1)
				decision = PLAYER;//Paper beats rock
			else if(computerChoice == 3)
				decision = COMPUTER;//Paper looses to Scissors 
		}
		else if (playerChoice == 3) { //Player Selects Scissors
			if(computerChoice == 1)
				decision = COMPUTER; //Scissors looses to Rock
			else if(computerChoice == 2)
				decision = PLAYER; //Scissors beats Paper
		}
		return decision;
	}
