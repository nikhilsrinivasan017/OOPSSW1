...
public class Hangman {
	 ...
	 *  return "P _ _ G _ _ M". There are spaces in this String between the characters but not at the start
	 *  or the end.<br><br>
	 *  
	...
	public String getCurrentGuessForDisplay(String currentGuess) {
		...
		for (int pos=0; pos < len; pos++) {
			if(currentGuess.charAt(pos) == '*')
				displayString = displayString.concat("_ ");
			else
				displayString = displayString.concat(currentGuess.charAt(pos)+" ");
		}
	...
	public String updateCurrentGuess(String target, String guess, char letter) {
		letter = Character.toLowerCase(letter);
		String newGuess = new String(guess);
		for(int pos = 0; pos < target.length(); pos++) {
			if(target.charAt(pos) == letter)
				newGuess = newGuess.substring(0,pos)+letter+newGuess.substring(pos+1);
		}
		return newGuess;
	}
